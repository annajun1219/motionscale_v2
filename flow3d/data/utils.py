...
def to_serializable(obj):
    """
    Convert metric dict to serializable dict for json outputs.
    """
...
def parse_cotracker3_track_info(visibility, confidence):
    """
    return:
...
def get_tracks_3d_for_query_frame(
    ...
    inv_Ks: torch.Tensor,
    c2ws: torch.Tensor,
    track_type: str = "bootstapir",
):
    """
    Get 3D tracks for one query frame (query_index) on all target frames (T frames).
    Project 2D tracks into 3D using depths, and return the world frame 3D tracks.

    Args:
        query_index (int): index of query frame in the list of target frames
        query_img (torch.Tensor): (H, W, 3), input image tensor
        tracks_2d (torch.Tensor): (N, T, 4), 2d tracks
        depths (torch.Tensor): (T, H, W), depth maps of target frames
        masks (torch.Tensor): (T, H, W), foreground masks
        inv_Ks (torch.Tensor): (T, 3, 3), inverse K
        c2ws (torch.Tensor): (T, 4, 4), c2w transformation matrix
        track_type (str): ["bootstapir", "cotracker3"], type of 2d tracks

    Returns:
        tracks_3d (torch.Tensor): (N1, T, 3), 3d tracks
        track_colors (torch.Tensor): (N1, 3), point colors
        visibles (torch.Tensor): (N1, T), visible masks
        invisibles (torch.Tensor): (N1, T), invisible masks
        confidences (torch.Tensor): (N1, T), confidence scores
        track_depths (torch.Tensor): (N1, T), 2d depth maps of track points
    """
    T, H, W = depths.shape
    ...
    # get visibility and confidence scores from raw network outputs, (T, N), (T, N), (T, N)
    if track_type in ["bootstapir", "tapir"]:
        visibles, invisibles, confidences = parse_tapir_track_info(occs, dists)
    elif track_type == "cotracker3":
        visibles, invisibles, confidences = parse_cotracker3_track_info(occs, dists)
    else:
        raise ValueError(f"Unknown track type: {track_type}")

    ## Unproject 2D tracks to 3D.
    # sample the depth map (T, H, W) at coordinates defined by 2D tracks (T, N, 2)
    # still use bilinear, use mask to remove edges
    track_depths = F.grid_sample(
        depths[:, None],  # (T, 1, H, W)
        normalize_coords(tracks_2d[:, None], H, W),  # (T, 1, N, 2)
        align_corners=True,
        padding_mode="border",
    )[:, 0, 0]  # (T, 1, 1, N) -> (T, N)
    # project 2d tracks into 3d using depths
    # Note: invisible points still use the depth
    tracks_3d = torch.einsum(
        "nij,npj->npi",
        inv_Ks,
        F.pad(tracks_2d, (0, 1), value=1.0),  # (T, N, 2) -> (T, N, 3)
    ) * track_depths[..., None]  # (T, N, 3)
    # convert to world frame
    tracks_3d = torch.einsum("nij,npj->npi", c2ws, F.pad(tracks_3d, (0, 1), value=1.0))[..., :3]  # (T, N, 3)

    ## Filter out out-of-mask tracks.
    # get the masks of track points
    is_in_masks = F.grid_sample(
        masks[:, None],  # (T, 1, H, W)
        normalize_coords(tracks_2d[:, None], H, W),  # (T, 1, N, 2)
        align_corners=True,
    )[:, 0, 0] == 1  # (T, N)
    ...
    # valid mask, should inside the mask on the query frame
    valid = is_in_masks[query_index]
    # valid if visible 5% of the time
    visible_counts = visibles.sum(0)
    valid = valid & (visible_counts >= min(int(0.05 * T), visible_counts.float().quantile(0.1).item()))

    ## Get track's color from the query frame.
    track_colors = F.grid_sample(
        query_img,  # (1, 3, H, W)
        normalize_coords(tracks_2d[query_index : query_index + 1, None], H, W),  # (1, 1, N, 2)
        align_corners=True,
        padding_mode="border",
    )[0, :, 0].T  # (1, 3, 1, N) -> (N, 3)
    ...
    return (
        ...
        track_depths[:, valid].swapdims(0, 1),
    )
...
def depth_map_aliasing(depth: torch.Tensor, atol: float = None, rtol: float = None, kernel_size: int = 3, mask: torch.Tensor = None) -> torch.BoolTensor:
    """
    Compute the map that indicates the aliasing of a depth map.
    The aliasing is defined as the pixels which neither close to the maximum nor the minimum of its neighbors.
...
def compute_depth_normal_mask(depths, thresh_rel=None, thresh_abs=None):
    """
    depths: [..., H, W, 1]
    Returns: mask [..., H, W, 1] with 1 = good pixel, 0 = bad pixel
    """
    # Compute finite differences along x/y
    dzdx = torch.abs(depths[..., 2:, 1:-1, :] - depths[..., :-2, 1:-1, :])
    dzdy = torch.abs(depths[..., 1:-1, 2:, :] - depths[..., 1:-1, :-2, :])
    depth_center = depths[..., 1:-1, 1:-1, :]

    # Initialize edge map
    edge = torch.zeros_like(depth_center, dtype=torch.bool)

    # Apply thresholds
    if thresh_abs is not None:
        edge |= (dzdx > thresh_abs) | (dzdy > thresh_abs)
    if thresh_rel is not None:
        edge |= ((dzdx / depth_center).nan_to_num_() > thresh_rel) | ((dzdy / depth_center).nan_to_num_() > thresh_rel)

    # pad to restore size
    edge = F.pad(edge, (0, 0, 1, 1, 1, 1), value=0.0)
    return edge