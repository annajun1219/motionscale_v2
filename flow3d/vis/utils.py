...
def draw_points_on_frame(img, points, colors, radii=2, mask=None, thickness=-1):
    num_pts = len(points)
    img = img.copy()
    h1, w1 = img.shape[:2]
    ...
    # visibility mask
    vis_mask = (
        (points[:, 0] >= 0) & (points[:, 0] <= w1 - 1) &
        (points[:, 1] >= 0) & (points[:, 1] <= h1 - 1)
    )
    points_vis = points[vis_mask]
    colors_vis = colors[vis_mask]
    mask_vis = mask[vis_mask] if mask is not None else np.zeros(len(points_vis), dtype=np.bool_)
    if type(radii) == int:
        radii = np.repeat(radii, len(points))
    radii_vis = radii[vis_mask]

    # draw points on each frame
    for i, point in enumerate(points_vis):
        color = tuple(colors_vis[i].tolist())  # rgb because the image is also rgb
        radius = int(round(radii_vis[i]))

        # draw a circle or draw a cross
        if not mask_vis[i]:
            img = cv2.circle(img, (int(round(point[0])), int(round(point[1]))), radius, color, thickness, cv2.LINE_AA)
        else:
            img = cv2.drawMarker(
                img, (int(round(point[0])), int(round(point[1]))), color, markerType=cv2.MARKER_TILTED_CROSS,
                markerSize=int(radius*2), thickness=int(radius*0.7), line_type=cv2.LINE_AA,
            )

    return img


def draw_track_points(imgs, tracks_2d, query_frame=0, masks=None, radius=3, colors=None, num_colors=128):
    if imgs.dtype == np.float32:
        imgs = (imgs * 255).astype(np.uint8)

    if colors is None:
        # get a default color map and sample some colors
        hsv = colormaps.get_cmap("hsv")
        color_map = np.array([hsv(i / num_colors)[:3] for i in range(num_colors)])
        color_map = (color_map * 255).astype(np.uint8)

        # set track point colors according to the angles towards the center
        points = tracks_2d[query_frame]
        center = np.median(points, axis=0)
        angles = np.arctan2(points[:, 1] - center[1], points[:, 0] - center[0])
        color_ids = (64 * angles / np.pi).astype(np.int32) % num_colors
        colors = color_map[color_ids]
    assert colors.shape[0] == tracks_2d.shape[1] and colors.shape[1] == 3

    # draw track points as circles on the video frames
    num_frames = len(imgs)
    video = []
    for i in range(num_frames):
        mask = masks[i] if masks is not None else None
        frame = draw_points_on_frame(imgs[i], tracks_2d[i], radius=radius, mask=mask, colors=colors)
        video.append(frame)
