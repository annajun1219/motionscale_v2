...
flow3d/graph_coupling_relative.py의 확장: node/edge feature(linear velocity
없음, 12차원 edge feature/NODE_FEAT_DIM은 baseline과 동일)는 그대로 유지한
채, message passing의 이웃 합치는 방식만 mean aggregation에서 학습된
multi-head attention(GAT류)으로 바꾼다. Linear/angular velocity는 이 파일의
범위 밖이다.

mean aggregation -> attention aggregation
-------------------------------------------
기존 _RelativeMessageLayer는
...
score 입력에 e_ij(=상대 translation/center/rotation)가 포함되어, 예를 들어
멀리 떨어진(혹은 크게 회전한) 이웃을 프레임 단위로 down-weight할 수 있다.
...
------------------------------------------------------
- so3_exp_map / compose_rotation / build_edge_index_from_edges_pt /
  cont_6d_to_rmat / rmat_to_cont_6d / CENTER_DIM / ROT_DIM / TRANSL_DIM /
  NODE_FEAT_DIM: 기존 flow3d/graph_coupling.py 구현을 그대로 가져다 쓴다.
- EDGE_FEAT_DIM(=12) / _build_directed_neighbor_edges: flow3d/
  graph_coupling_relative.py 구현을 그대로 가져다 쓴다 (node/edge feature
  구성과 topology 구성은 이 확장으로 바뀌지 않는다). linear velocity, vel_scale,
  coarse_vel, _compute_vel_scale, NODE_FEAT_DIM_VEL/EDGE_FEAT_DIM_VEL, 15차원
  feature는 이 파일의 범위 밖이며 등장하지 않는다.
...
이 파일에서 제공하는 것
------------------------
- RelativeAttentionClusterGraphGNN: RelativeClusterGraphGNN과 같은 API(node
  feature -> (omega, delta_t))를 갖지만, message aggregation이 mean이 아니라
  multi-head attention인 GNN.
- RelativeAttentionGraphCorrectedScalableMotionBases: baseline
  RelativeGraphCorrectedScalableMotionBases와 동일한 API
...
__all__ = [
    ...
    "compose_rotation",
    "build_edge_index_from_edges_pt",
    "RelativeAttentionClusterGraphGNN",
...
class _RelativeAttentionMessageLayer(nn.Module):
    """Fixed-topology relative message passing, 1 layer, residual -- same
    message MLP as flow3d/graph_coupling_relative.py's _RelativeMessageLayer,
    but aggregated with learned multi-head attention instead of a uniform
    (1/in_degree) mean:
    ...
    def _compute_attention(
        ...
        :param edge_feat: (E, B, EDGE_FEAT_DIM) relative geometry per edge
    ...
    def forward(
        ...
        edge_feat: (E, B, EDGE_FEAT_DIM) relative geometry feature per directed
                   edge (shared across layers within one forward call)
        """
        C, B, H = h.shape
        if edge_index_dir.numel() == 0:
...
class RelativeAttentionClusterGraphGNN(nn.Module):
    """RelativeClusterGraphGNN과 같은 API(node feature -> (omega, delta_t))를
    갖는 relative-message GNN이지만, 이웃 message aggregation이 mean이 아니라
    multi-head attention이다. node/edge feature(linear velocity 없음, 12차원
    edge feature)는 baseline과 완전히 동일하다.
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [
                _RelativeAttentionMessageLayer(hidden_dim, num_heads=num_heads)
    ...
    def _compute_edge_features(
        ...
        를 만든다 (flow3d/graph_coupling_relative.py와 동일). 한 forward call
        안에서 한 번만 계산되어 모든 layer에 재사용된다.
        """
...
class RelativeAttentionGraphCorrectedScalableMotionBases(ScalableMotionBases):
    """ScalableMotionBases + relative-message-with-attention graph-aware
    coarse transform correction. baseline RelativeGraphCorrectedScalableMotionBases와
    API가 동일한 드롭인 대체이며, 차이는 내부 GNN이
    RelativeAttentionClusterGraphGNN(attention aggregation)이라는 점뿐이다.
    """
    ...
    def __init__(
        ...
        self.gnn = RelativeAttentionClusterGraphGNN(
    ...
    def from_scalable_motion_bases(
        ...
        """기존 (초기화된) ScalableMotionBases로부터 relative+attention
    ...
    def has_gnn_state(state_dict: dict, prefix: str) -> bool:
        ...
        RelativeAttentionGraphCorrectedScalableMotionBases로 저장된 것인지
    ...
    def init_from_state_dict(
        ...
        """체크포인트만으로 RelativeAttentionGraphCorrectedScalableMotionBases를
        ...
        (num_heads, 3*head_dim) 모양으로 저장되므로 shape[0]가 곧 num_heads이다).
        directed edge 버퍼는 num_clusters만으로 placeholder를 만들 수 있고,
...
if __name__ == "__main__":
    ...
    #   5. num_heads=1 also runs.
    #   6. save -> init_from_state_dict round-trip reproduces the same output.
    torch.manual_seed(0)
    ...
    graph_bases = RelativeAttentionGraphCorrectedScalableMotionBases.from_scalable_motion_bases(
        baseline,
        edge_index=edge_index,
        gnn_hidden_dim=gnn_hidden_dim,
        gnn_num_layers=gnn_num_layers,
    ...
    print(f"[coarse]  max |baseline - relative_attention_graph_corrected(zero-init)| = {max_diff_coarse:.3e}")
    ...
    print(f"[full]    max |baseline - relative_attention_graph_corrected(zero-init)| = {max_diff:.3e}")
    ...
    # in dL/dW, same reasoning as flow3d/graph_coupling_relative.py.
    ...
    # --- 5. num_heads=1 smoke test ---
    one_head_bases = RelativeAttentionGraphCorrectedScalableMotionBases.from_scalable_motion_bases(
    ...
    toy_bases = RelativeAttentionGraphCorrectedScalableMotionBases.from_scalable_motion_bases(
    ...
    toy_node_feat = torch.cat(
        [toy_coarse_rot_6d, toy_coarse_transl, toy_center_feat], dim=-1
    )
    toy_h = toy_gnn.encoder(toy_node_feat)
    toy_edge_feat = toy_gnn._compute_edge_features(
        toy_coarse_rot_6d, toy_coarse_transl, toy_centers
    )
    ...
    restored = RelativeAttentionGraphCorrectedScalableMotionBases.init_from_state_dict(
    ...
    print(
        ...
        f"num_heads match = {num_heads_match} "
